...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const pharmaTemplateData = [
      {
        ...
        "Maximum Stock": 500,
        "Reorder Point": 20,
        "Batch Number": "B-2026-001 (Optional)",
        "Batch Expiration Date": "2027-06-30 (Optional, needed for expiry tracking/alerts on this stock)",
    ...
    const instructionsWorksheet = XLSX.utils.json_to_sheet([
      ...
      {
        Field: "Batch Number / Batch Expiration Date",
        Required: "No, but recommended",
        Description:
          "If both are filled, Current Stock is recorded as a tracked batch with that expiry date — this is what powers expiry alerts and FEFO (soonest-expiry-first) selling. Left blank, the stock is still added but won't show up in expiry tracking until a batch is received for it later.",
      },
